...
the pass aborts. The outer loop restarts each subsequent pass from the
...
impacted moves are re-evaluated, and the move-audit pattern
(\texttt{doc/move\_audit\_pattern.md}) replays each move in a
sandboxed copy before commit to catch any invariant violation at
write time rather than discovering it downstream.

\textbf{Parallelism.} Init, FM, and evolution tasks run on up to 8
threads behind a work-stealing task queue; task granularity is a full
FM outer loop, so lock contention on the diversity pool is negligible.
All shared state (cost cache, pool) is either lock-free or
