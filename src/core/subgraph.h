...
public:
  ...
  // Find the TileConfig with minimum latency (exhaustive over power-of-2
  // divisors).
...
private:
  ...
  std::vector<int64_t>
      all_out_widths_; // all op output widths (for w validation)
  std::vector<int64_t>
      all_out_heights_; // all op output heights (for h validation)
  ...
  std::vector<BoundaryTensorInfo>
      boundary_tensor_info_; // built at create() time

  std::vector<int64_t> ws_cand_; // Divisor candidate array for 'w'
  std::vector<int64_t> hs_cand_; // Divisor candidate array for 'h'
  std::vector<int64_t> ks_cand_; // Divisor candidate array for 'k'
};