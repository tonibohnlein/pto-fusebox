...
static bool step_depends_on(const ScheduleStep& si, const ScheduleStep& sj, const DAG& dag) {
...
static std::set<size_t> filter_retain(const std::set<size_t>& retain, const Subgraph& sg) {
    std::set<size_t> r;
    for (auto t : retain)
        if (sg.boundary_inputs().count(t) || sg.boundary_outputs().count(t) || t == sg.sink_tensor())
            r.insert(t);
    return r;
}

static bool is_connected_without(const std::set<size_t>& ops, size_t rm, const DAG& dag, size_t n) {
    if (ops.size() <= 2) return false;
    size_t seed = SIZE_MAX;
    for (auto op : ops) if (op != rm) { seed = op; break; }
    ...
    while (!q.empty()) {
        ...
        for (auto v : dag.op_preds[u])
            if (v != rm && ops.count(v) && !vis[v]) { vis[v]=true; clr.push_back(v); q.push_back(v); cnt++; }
        for (auto v : dag.op_succs[u])
            if (v != rm && ops.count(v) && !vis[v]) { vis[v]=true; clr.push_back(v); q.push_back(v); cnt++; }
    }
    for (auto i : clr) vis[i] = false;
...
static std::vector<size_t> find_steps_of_tensor(const std::vector<ScheduleStep>& steps, size_t t) {
    std::vector<size_t> r;
    for (size_t i = 0; i < steps.size(); i++)
...
struct SolState {
    ...
    void rebuild_all() {
        size_t n = steps.size();
        ret_entering.resize(n); cost.resize(n); gen.resize(n, 0);
        total = 0;
        std::set<size_t> cur;
    ...
    void rebuild_from(size_t idx) {
        size_t n = steps.size();
        ret_entering.resize(n); cost.resize(n, 0); gen.resize(n, 0);
        std::set<size_t> cur;
...
static SolutionMove best_move_for_op(SolState& state, size_t op,
    ...
    for (auto p : dag.op_preds[op]) if (!si_set.count(p)) { is_border = true; break; }
    if (!is_border) for (auto s : dag.op_succs[op]) if (!si_set.count(s)) { is_border = true; break; }
    ...
    if (is_border && si_set.size() > 1) {
        for (int dir = -1; dir <= 1; dir += 2) {
            ...
            std::set<size_t> new_src = si_set; new_src.erase(op);
            std::set<size_t> new_dst = dj_set; new_dst.insert(op);
            ...
            if (!cd.feasible) continue;
            double saving = (state.cost[si] + state.cost[sj]) - (cs.latency + cd.latency);
            if (saving > -floor && saving > best.saving) {
                best.type = SolutionMove::STEAL; best.step_a = si; best.step_b = sj;
                best.op = op; best.saving = saving;
            }
        }
    }
    ...
    if (is_border) {
        for (int dir = -1; dir <= 1; dir += 2) {
            ...
            if (!cm.feasible) { retain.clear(); cm = sg->best_cost(state.ret_entering[lo], {}); }
            if (!cm.feasible) continue;
            double saving = (state.cost[si] + state.cost[sj]) - cm.latency;
            if (saving > -floor && saving > best.saving) {
                best.type = SolutionMove::MERGE; best.step_a = lo; best.step_b = hi;
                best.op = op; best.saving = saving;
            }
        }
    }
    ...
    if (is_border) {
        for (int dir = -1; dir <= 1; dir += 2) {
            ...
            if (!ce.feasible) continue;
            double saving = state.cost[sj] - ce.latency;
            if (saving > -floor && saving > best.saving) {
                best.type = SolutionMove::RECOMPUTE; best.step_a = sj; best.step_b = si;
                best.op = op; best.saving = saving;
            }
        }
    }
    ...
    if (is_border && si_set.size() >= 2 && is_connected_without(si_set, op, dag, prob.num_ops())) {
        std::set<size_t> remainder = si_set; remainder.erase(op);
        ...
        if (sg_r && sg_s) {
            ...
            if (cr.feasible && cs.feasible) {
                ...
                if (saving > -floor && saving > best.saving) {
                    best.type = SolutionMove::EJECT; best.step_a = si;
                    best.op = op; best.saving = saving;
                }
            }
        }
    ...
    if (!is_border && si_set.size() >= 3 && si_set.size() <= 15) {
        ...
        if (er.feasible && er.saving > -floor && er.saving > best.saving) {
            best.type = SolutionMove::INTERNAL_EJECT; best.step_a = si;
            best.op = op; best.saving = er.saving;
        }
    }
        ...
        auto try_split = [&](size_t a, size_t b) {
            ...
            for (int order = 0; order < 2; order++) {
                ...
                for (auto t : first->boundary_outputs())
                    if (second->boundary_inputs().count(t) && prob.retainable_tensors.count(t))
                        br.insert(t);
                auto c1 = first->best_cost(state.ret_entering[si], br);
                if (!c1.feasible) { br.clear(); c1 = first->best_cost(state.ret_entering[si], {}); }
                if (!c1.feasible) continue;
                auto sr2 = filter_retain(state.steps[si].retain_these, *second);
                auto c2 = second->best_cost(br, sr2);
                if (!c2.feasible) c2 = second->best_cost(br, {});
                if (!c2.feasible) continue;
                double saving = state.cost[si] - (c1.latency + c2.latency);
                if (saving > -floor && saving > best.saving) {
                    best.type = SolutionMove::SPLIT; best.step_a = si;
                    best.op = a; best.op2 = b; best.saving = saving;
                }
            }
        };
        for (auto s : dag.op_succs[op]) if (si_set.count(s)) try_split(op, s);
        for (auto p : dag.op_preds[op]) if (si_set.count(p)) try_split(p, op);
    }
...
static SolutionMove best_move_for_tensor(SolState& state, size_t t,
    ...
    SolutionMove best;
    const auto& prob = *state.prob;
    if (locked_tensors.count(t)) return best;
    if (!prob.retainable_tensors.count(t)) return best;
    ...
    for (size_t i = 0; i + 1 < state.size(); i++) {
        ...
        if (si.retain_these.count(t)) continue;  // already retained
        ...
        if (!si_boundary) continue;
        // Step i+1 must benefit from having t
        if (!sj.subgraph.boundary_inputs().count(t)) continue;

        auto new_ret = si.retain_these; new_ret.insert(t);
        auto new_ent = state.ret_entering[i + 1]; new_ent.insert(t);
        ...
        if (saving > -floor && saving > best.saving) {
            best.type = SolutionMove::RETAIN_ADD; best.step_a = i;
            best.tensor = t; best.saving = saving;
        }
    }

    // RETAIN_REMOVE: find steps where removing t from retain helps
    for (size_t i = 0; i + 1 < state.size(); i++) {
        if (!state.steps[i].retain_these.count(t)) continue;
        auto& si = state.steps[i];
        auto& sj = state.steps[i + 1];
        auto new_ret = si.retain_these; new_ret.erase(t);
        auto new_ent = state.ret_entering[i + 1]; new_ent.erase(t);
        auto ci = si.subgraph.best_cost(state.ret_entering[i], new_ret);
        if (!ci.feasible) continue;
        auto cj = sj.subgraph.best_cost(new_ent, sj.retain_these);
        if (!cj.feasible) continue;
        double saving = (state.cost[i] + state.cost[i + 1]) - (ci.latency + cj.latency);
        if (saving > -floor && saving > best.saving) {
            best.type = SolutionMove::RETAIN_REMOVE; best.step_a = i;
            best.tensor = t; best.saving = saving;
        }
    }
...
static std::pair<size_t,size_t> apply_move(SolState& state, const SolutionMove& m) {
    ...
    case SolutionMove::STEAL: {
        ...
        std::set<size_t> ns(src.subgraph.ops().begin(), src.subgraph.ops().end()); ns.erase(m.op);
        std::set<size_t> nd(dst.subgraph.ops().begin(), dst.subgraph.ops().end()); nd.insert(m.op);
        auto ss = Subgraph::create(prob, dag, {ns.begin(), ns.end()});
        auto sd = Subgraph::create(prob, dag, {nd.begin(), nd.end()});
        if (!ss || !sd) return {SIZE_MAX, 0};
        ...
        if (!cs.feasible || !cd.feasible) return {SIZE_MAX, 0};
        src.subgraph = std::move(*ss); src.config = cs.config; src.retain_these = sr;
        dst.subgraph = std::move(*sd); dst.config = cd.config; dst.retain_these = dr;
    ...
    case SolutionMove::MERGE: {
        ...
        std::vector<size_t> merged; std::set<size_t> seen;
        for (auto o : oi) if (seen.insert(o).second) merged.push_back(o);
        for (auto o : oj) if (seen.insert(o).second) merged.push_back(o);
        auto sg = Subgraph::create(prob, dag, merged);
        ...
        if (!cm.feasible) { ret.clear(); cm = sg->best_cost(state.ret_entering[m.step_a], {}); }
    ...
    case SolutionMove::RECOMPUTE: {
        ...
        if (!ce.feasible) return {SIZE_MAX, 0};
        state.steps[m.step_a].subgraph = std::move(*sg);
    ...
    case SolutionMove::EJECT: {
        ...
        std::set<size_t> rem = si_set; rem.erase(m.op);
        ...
        si.subgraph = std::move(*sg_r); si.config = cr.config; si.retain_these = rr;
        ScheduleStep ns; ns.subgraph = std::move(*sg_s); ns.config = cs.config;
    ...
    case SolutionMove::INTERNAL_EJECT: {
        ...
        if (!sg_sin) return {SIZE_MAX, 0};
        auto cs = sg_sin->best_cost({}, {});
        if (!cs.feasible) return {SIZE_MAX, 0};
        ScheduleStep ss; ss.subgraph = std::move(*sg_sin); ss.config = cs.config;
        ...
        for (auto& comp : comps) {
            ...
            ScheduleStep ns; ns.subgraph = std::move(*sg); ns.config = cc.config;
            new_steps.push_back(std::move(ns));
        }
        state.steps.erase(state.steps.begin() + m.step_a);
        for (size_t k = 0; k < new_steps.size(); k++)
            state.steps.insert(state.steps.begin() + m.step_a + k, std::move(new_steps[k]));
        return {m.step_a, state.size()};
    }
    case SolutionMove::SPLIT: {
        ...
        if (!c_a.feasible) { br.clear(); c_a = sg_a->best_cost(state.ret_entering[m.step_a], {}); }
        ...
        ScheduleStep ns; ns.subgraph = std::move(*sg_b); ns.config = c_b.config; ns.retain_these = vr;
        state.steps.insert(state.steps.begin() + m.step_a + 1, std::move(ns));
        return {m.step_a, state.size()};
    }
...
static void generate_step_moves(SolState& state, size_t si, SolMoveHeap& heap,
                                 double floor,
                                 Clock::time_point deadline = Clock::time_point::max()) {
    if (si >= state.size()) return;
    ...
    for (auto op : state.steps[si].subgraph.ops()) {
        ...
        if (m.valid() && m.saving > -floor) {
            m.gen_a = gen_i;
            m.gen_b = (m.step_b != SIZE_MAX && m.step_b < state.size()) ? state.gen[m.step_b] : -1;
            heap.push(m);
        }
    }
...
static void bump_affected(SolState& state, size_t lo, size_t hi, bool structural) {
    if (structural) {
        ...
        for (size_t i = lo; i < std::min(hi, state.size()); i++)
            state.bump(i);
        // Also bump neighbors
        if (lo > 0) state.bump(lo - 1);
        if (hi < state.size()) state.bump(hi);
    }
}
...
static bool is_stale(const SolutionMove& m, const SolState& state) {
    if (m.step_a >= state.size()) return true;
    if (m.gen_a != -1 && m.gen_a != state.gen[m.step_a]) return true;
    if (m.step_b != SIZE_MAX) {
        if (m.step_b >= state.size()) return true;
        if (m.gen_b != -1 && m.gen_b != state.gen[m.step_b]) return true;
    }
    // For RETAIN moves, check gen_b (next step)
    if ((m.type == SolutionMove::RETAIN_ADD || m.type == SolutionMove::RETAIN_REMOVE) &&
        m.step_a + 1 < state.size() && m.gen_b != -1 && m.gen_b != state.gen[m.step_a + 1])
        return true;
    return false;
}
...
struct SolActiveSet {
    ...
    void activate_op(SolState& state, size_t op) {
        if (active_ops.count(op) || locked_ops.count(op)) return;
        if (Clock::now() >= deadline) return;
    ...
    void activate_tensor(SolState& state, size_t t) {
        if (active_tensors.count(t) || locked_tensors.count(t)) return;
        if (!state.prob->retainable_tensors.count(t)) return;
        if (Clock::now() >= deadline) return;
    ...
    void activate_step(SolState& state, size_t si) {
        ...
        for (auto op : step.subgraph.ops()) activate_op(state, op);
        for (auto t : step.subgraph.boundary_inputs()) activate_tensor(state, t);
        for (auto t : step.subgraph.boundary_outputs()) activate_tensor(state, t);
        activate_tensor(state, step.subgraph.sink_tensor());
        for (auto t : step.retain_these) activate_tensor(state, t);
    }
    ...
    void update_affected(SolState& state, size_t lo, size_t hi) {
        ...
        for (size_t i = lo; i < std::min(hi, state.size()); i++) {
            for (auto op : state.steps[i].subgraph.ops()) affected_ops.insert(op);
            for (auto t : state.steps[i].subgraph.boundary_inputs()) affected_tensors.insert(t);
            for (auto t : state.steps[i].subgraph.boundary_outputs()) affected_tensors.insert(t);
            affected_tensors.insert(state.steps[i].subgraph.sink_tensor());
        }
        // Neighbors: one step before lo and after hi
        if (lo > 0) {
            for (auto op : state.steps[lo-1].subgraph.ops()) affected_ops.insert(op);
            for (auto t : state.steps[lo-1].subgraph.boundary_outputs()) affected_tensors.insert(t);
        }
        if (hi < state.size()) {
            for (auto op : state.steps[hi].subgraph.ops()) affected_ops.insert(op);
            for (auto t : state.steps[hi].subgraph.boundary_inputs()) affected_tensors.insert(t);
        }

        for (auto& e : entries) {
            if (Clock::now() >= deadline) break;
            if (e.is_tensor) {
                if (locked_tensors.count(e.id)) continue;
                if (affected_tensors.count(e.id))
                    e.move = best_move_for_tensor(state, e.id, locked_tensors, floor);
            } else {
                if (locked_ops.count(e.id)) continue;
    ...
    std::optional<SolutionMove> pop_best() {
        ...
        for (size_t i = 0; i < entries.size(); i++) {
            auto& e = entries[i];
            if (e.is_tensor && locked_tensors.count(e.id)) continue;
            if (!e.is_tensor && locked_ops.count(e.id)) continue;
        ...
        auto result = e.move;
        if (e.is_tensor) locked_tensors.insert(e.id);
        else locked_ops.insert(e.id);
        return result;
    }
};
...
std::vector<ScheduleStep> solution_greedy_descent(const Problem& prob, const DAG& dag,
    ...
    if (state.size() > 30) max_init = state.size() / 2;
    ...
    while (!heap.empty()) {
        ...
        if (m.saving <= 0.01) break;

        // Staleness check
        if (is_stale(m, state)) continue;
...
SolutionFMPassResult solution_fm_pass(const Problem& prob, const DAG& dag,
    ...
    for (size_t t = 0; t < prob.num_tensors(); t++)
        if (prob.retainable_tensors.count(t)) all_tensors.push_back(t);
    std::vector<size_t> all_ops;
    for (size_t i = 0; i < state.size(); i++)
        for (auto op : state.steps[i].subgraph.ops()) all_ops.push_back(op);
    ...
    for (int iter = 0; iter < 200; iter++) {
        ...
        if (!m_opt || m_opt->saving < -floor) break;
        ...
        if (m.type == SolutionMove::RETAIN_ADD && m.tensor != SIZE_MAX) {
            ...
            if (prod >= 0) active.locked_ops.insert((size_t)prod);
            ...
            if (m.step_a + 1 < state.size()) {
                for (auto op : state.steps[m.step_a + 1].subgraph.ops()) {
                    for (auto t : prob.ops[op].inputs) {
                        if (t == m.tensor) { active.locked_ops.insert(op); break; }
                    }
                }
            }
